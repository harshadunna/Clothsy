...
export default function CartItem({ item, showButton }) {
  ...
  const discountPercent = item?.product?.discountPersent || 0;
  ...
  const handleUpdate = (delta) => {
    console.log("Update qty:", quantity + delta);
    // dispatch(updateCartItem({ data: { quantity: quantity + delta }, cartItemId: item?.id, jwt }));
  };
  ...
  return (
    <motion.div
      ...
      style={{
        borderColor: "#e8ddd5",
        boxShadow: "0 2px 16px rgba(180,140,90,0.07)",
      }}
    >
      <div className="flex gap-0">
        ...
        {/* ── Image ── */}
        <div
          className="w-28 sm:w-36 shrink-0 relative overflow-hidden"
          ...
          <div>
            <div className="flex justify-between items-start gap-2">
              <div>
                ...
                <h3
                  className="font-bold text-base mt-0.5 leading-snug"
                  style={{ color: "#1a1109" }}
                >
                  {title}
                </h3>
              ...
              <div className="text-right shrink-0">
                ...
                <p className="text-xs line-through mt-0.5" style={{ color: "#b5a89a" }}>
                  ₹{price}
                </p>
              </div>
            </div>
