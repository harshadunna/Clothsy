...
function FormInput({
  label, name, id, autoComplete, type = "text",
  required = true, colSpan = 1, defaultValue = "",
}) {
  return (
...
export default function DeliveryAddress({ handleNext }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (editingAddress) {
      await dispatch(updateAddress(editingAddress.id, address));
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-24">
      ...
      <div className="lg:col-span-7">
        <AnimatePresence mode="wait">
          ...
          {/* Show form when: explicitly opened OR no saved addresses yet */}
                ...
                <div className="flex justify-between items-end border-b border-outline-variant/30 pb-2 mb-8">
                  ...
                  {savedAddresses.length > 0 && (
                    <button
                      ...
                      className="text-[10px] font-label uppercase tracking-widest text-primary border-b border-primary pb-0.5"
                    >
                      Cancel
                    </button>
                  )}
                ...
                <form onSubmit={handleSubmit} className="space-y-10">
                  ...
                  <div className="pt-8 flex justify-end">
                    <button
                      ...
                      className="bg-on-background text-surface px-12 py-5 font-label text-xs font-black uppercase tracking-[0.3em] hover:bg-primary transition-all duration-300 disabled:opacity-50"
                    >
                      {order?.loading
                        ? "PROCESSING..."
                        : editingAddress
                ...
                <div className="space-y-6">
                  {savedAddresses.map((item) => (
                    <div key={item.id} className="border border-outline-variant/30 p-6 relative">
                      ...
                      />
                      <button
                        disabled={order?.loading && selectedAddressId === item.id}
                      ...
                      >
                        {order?.loading && selectedAddressId === item.id
                          ? "PROCESSING..."
      ...
      {/* Right: Order summary placeholder */}
      <div className="hidden lg:block lg:col-span-5">
        <div className="bg-surface-container-low p-10 sticky top-32">
          <h3 className="font-headline text-3xl italic tracking-tighter text-outline mb-10">
            Order Summary
          </h3>
          <p className="font-label text-[10px] uppercase tracking-widest text-outline">
            Summary available in the next step.
          </p>
        </div>
      </div>
    </div>
